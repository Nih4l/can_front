        ...
        {dates.slice(0, 3).map((dateObj, index) => (
          ...
          >
            <div className="text-center w-[20%] mt-2 ">
              <div className="text-[12px] text-[#7E7E7E] font-semibold">{dateObj.day}</div>
              <div className='text-[12px] font-semibold text-black'>{dateObj.date === today.getDate() ? today.getDate() : dateObj.date}</div>
            </div>
            
            <div className={` flex flex-col  `}>
              <div className={` p-4 pr-12 h-14  flex items-center rounded-md flex-row gap-2 transition-all duration-500  ${dateObj.date === today.getDate() ? 'text-black bg-[#F5F5F5]' : 'text-black bg-[#F5F5F5]'} `}>
